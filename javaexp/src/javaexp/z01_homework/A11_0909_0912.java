...
//[1단계:확인] 2. [매개변수연습] driving메서드에 매개변수로 1~3개를 각각 선언하되,
		...
		class Car{
			...
			void driving(int distance,String name) { //매개변수의 타입순서가 다름..
				this.name = name;
				this.distance=distance;
				this.driving();
			}
...
[1단계:확인] 4. [매개변수+리턴] Member 클래스 선언하고, regdate()메서드로
	...
	String pass;
	double point;
	String regdata(String name, String id, String pass) {
		this.name=name;
		this.id=id;
		this.pass=pass;
...
[1단계:확인] 1. [매개변수+필드] Caffee 클래스 선언, 
	...
	public void orderCoffee(String cname, int price, int count) {
		this.cname=cname;
		this.price=price;
		this.count=count;
	}
	...
	public String showAll() {
		String ret="커피종류:"+this.cname+", 가격:"+this.price+", 갯수:"+this.count+", 총액:"
...
[1단계:확인] 2. [필드+로직처리] Gugu 클래스 선언
	...
	void showResult2() {
		for(int grade=this.start;grade<=this.end;grade++) {
			...
			for(int cnt=1; cnt<=9;cnt++) {
				System.out.println(grade+"X"+this.cnt+"="+this.grade*cnt);	
			}
		}
	}
}
...
[1단계:확인] 3. [필드+로직처리] Bus 클래스 선언
	...
	private int pay;
	public void vasicInfo(int no, String fromTo, int pay) {
		this.no=no;
		this.fromTo=fromTo;
		this.pay=pay;
	}
	public int takeBus(int cnt) {
