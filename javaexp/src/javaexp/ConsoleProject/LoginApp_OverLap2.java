package javaexp.ConsoleProject;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.Scanner;

public class LoginApp_OverLap2 {

	private static Scanner scanUser = new Scanner(System.in);
	private static UserInfo4[] user = new UserInfo4[10]; //
	...
	private static void SignUp() {
		...
		for(int cnt=0;cnt<10;cnt++) {
			if(user[cnt].getUserName()==null) {
				count++;
			}
		}System.out.println("가입 가능한 인원:"+count);
					...
					if (OverLapPass == 0) {
						...
						if (OverLapNick == 0) {
							...
							if(SignPhoneNum.length()!=11) {
								System.out.println("휴대폰 번호 11자리를 입력해주세요");
								OverPNum = 1;
							}else {
								...
								System.out.println("\n<회원가입이 완료 되었습니다!!>");
								user[cnt].setUserName(SignId);// 입력된 SignNick는 user[0] 배열의 setUserNic에 할당된다.
								System.out.println("입력하신ID: " + SignId);
								user[cnt].setUserPhoneNum(SignPhoneNum);// 입력된 SignPhoneNum는 user[0] 배열의 setUserNic에 할당된다.
								System.out.println("전화번호: " + SignPhoneNum);
								user[cnt].setUserPass(SignPass); // 입력된 SginId는 user[0] 배열의 setUserName에 할당된다
								user[cnt].setUserNic(SignNick);// 입력된 SignPass는 user[0] 배열의 setUserPass에 할당된다.
								System.out.println("\n");						
							}
							}
						}
	...
	private static void Login_User() {
		...
		for (int logcnt = 0; logcnt < 10; logcnt++) {
			if (logId.equals(user[logcnt].getUserName()) && logPass.equals(user[logcnt].getUserPass())) {
				System.out.println("<로그인이 완료, 환영합니다!>");
...
//======================회원 내용 조회 부분======================================		
	...
	private static void LookUp_User() {
		// ex) for문(65번줄)을 통해 회원 내용을 출력하는데, 만약 회원 탈퇴를 진행하여 user[0]의 회원 내용이 지워져 null이
		// 표시되면
...
//==================회원 탈퇴====================================================================================
	...
	public static void main(String[] args) {
		for (int cnt = 0; cnt < 10; cnt++) {
			user[cnt] = new UserInfo4(); // 각 배열을 초기화 시켜줌으로써 NullPointException에러 방지
		}
		boolean end = true;
		do {
			System.out.println("*-----------------------------------------------*");
...
class UserInfo4 {
	...
	public UserInfo4(String userName, String userPass, String userNic, String userPhoneNum) {
		UserName = userName;
		UserPass = userPass;
		UserNic = userNic;
