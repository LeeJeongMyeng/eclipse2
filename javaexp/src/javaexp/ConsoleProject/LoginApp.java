	...
	private static void SignUp() {
		...
		if (OverLapId == 0) {
			...
			if (OverLapPass == 0) {
				...
				if (OverLapNick == 0) {
					user[Usernumber].setUserNic(SignNick);// 입력된 SignNick는 user[0] 배열의 setUserNic에 할당된다.
					System.out.println("입력하신ID: " + SignId);
					user[Usernumber].setUserName(SignId); // 입력된 SginId는 user[0] 배열의 setUserName에 할당된다
					user[Usernumber].setUserPass(SignPass);// 입력된 SignPass는 user[0] 배열의 setUserPass에 할당된다.
	...
	private static void LookUp_User() {
		...
		try {
			...
			for (int cnt = 0; cnt < 10; cnt++) {
				...
				if (!(user[cnt].getUserNic()).equals(null)) {
...
class UserInfo {
	...
	public UserInfo(String userName, String userPass, String userNic) {
